...
import { View, Text, Button, ScrollView, ActivityIndicator, StyleSheet, Image } from 'react-native';
...
export default function RecipeDetail() {
  ...
  useEffect(() => {
    fetchRecipeById(id).then(res => setMeal(res.data.meals[0]));
  }, [id]);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Button title={isFav ? 'Remove Favorite' : 'Add to Favorites'} onPress={() => isFav ? removeFavorite(id) : addFavorite(meal)} />
    </ScrollView>
  );
}
